...
class DescobridorLocais:
    ...
    def buscar_extremos(self, min_lat: float, max_lat: float, min_lon: float, max_lon: float,
        ...
        while True:
            pois = [
                ...
                if self.geo.haversine(centro_lat, centro_lon, e.get("lat", 0), e.get("lon", 0)) <= raio_km
            ]
    ...
    def buscar_bairros(self, c1, c2, e1, e2, cidade: str, uf: str, diametro_km: float,
                       usadas: set = None):
        ...
        m1_lat, m1_lon, nome_poi = self._warp_para_poi(
            (e1.lat + c1.lat) / 2, (e1.lon + c1.lon) / 2, usadas)
        if nome_poi:
            nome_m1 = nome_poi
        ...
        m2_lat, m2_lon, nome_poi = self._warp_para_poi(
            (e2.lat + c2.lat) / 2, (e2.lon + c2.lon) / 2, usadas)
        if nome_poi:
            nome_m2 = nome_poi
